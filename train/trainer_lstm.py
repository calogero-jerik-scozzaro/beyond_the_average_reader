...
class TrainerLSTM:
    ...
    def train(self):
        ...
        for epoch in range(self.config['epochs']):
            ...
            stop, self.min_loss, self.best_model_state, self.epochs_no_improve = early_stopping(self.model, val_loss, self.min_loss, self.epochs_no_improve, self.best_model_state)
            if stop:
                break
    ...
    def fine_tune(self, train_loader_ft,model):
        ...
        self.optim = AdamW(self.model.parameters(), lr=5e-3)
        ...
        for epoch in range(self.config["ft_epochs"]):
            ...
            self.scheduler.step()
            stop, self.min_loss, self.best_model_state, self.epochs_no_improve = early_stopping(self.model, epoch_loss, self.min_loss, self.epochs_no_improve, self.best_model_state)
            if stop:
                break
